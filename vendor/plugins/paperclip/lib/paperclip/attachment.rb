module Paperclip
  ...
  class Attachment
    ...
    def image?
      /image\/.*/.match(self.content_type)
    end
    
    # Only attachment that are thumbnailable
    # will have their thumbnails generated
    def thumbnailable?
      self.image?
    end
    ...
    def post_process_styles #:nodoc:
      unless self.thumbnailable?
        (@errors[:processing] ||= []) << "is not thumbnailable!" if @whiny
        return
      end
      @styles.each do |name, args|
        begin
